...
class UNet(nn.Module):
    ...
    def forward(self, x, t, y=None):
        ...
        if y is not None:
            ...
            t = t + self.label_emb(y)#[:,None,None,:].repeat(1,1,x.shape[-2],x.shape[-1])
...
class Classifier(nn.Module):
    def __init__(self, img_size=16, c_in=3, labels=5, time_dim=256, device="cuda", channels=32):
        super().__init__()
        # Classify noisy images of the dataset. Don’t forget to add the timestep as input. Use the UNet class.
        self.device = device
        self.time_dim = time_dim
        self.inc = DoubleConv(c_in, channels)
        self.down1 = Down(channels, channels*2,  emb_dim=time_dim)
        ...
        self.bot1 = DoubleConv(channels*4, channels*8) # optional, if included more parameters, also in forward
        self.bot2 = DoubleConv(channels*8, channels*8)
        self.bot3 = DoubleConv(channels*8, channels*4)

        self.final = nn.Linear(channels*4*img_size // 8*img_size // 8, labels)
